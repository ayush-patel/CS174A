...
window.Subdivision_Cylinder = window.classes.Subdivision_Cylinder =
    class Subdivision_Cylinder extends Shape {
        constructor(max_subdivisions) {
            super("positions", "normals");

            // Generate (x,y) points defining circular "caps" of cylinder, subdivided as necessary.
            let points = [];
            let pointsCount = 3*2**max_subdivisions;
            let theta, dtheta = (2*Math.PI)/pointsCount;
            for (let i = 0; i < pointsCount; i++) {
                theta = i*dtheta;
                points.push( [Math.cos(theta), Math.sin(theta)] );
            }

            // Build position vectors from calculated points and construct cylinder walls.
            // Manually add first slice.
            this.positions.push(...Vec.cast( [points[0][0], points[0][1], 0], [points[0][0], points[0][1], 1] ));
            this.normals.push(...Vec.cast( [points[0][0], points[0][1], 0], [points[0][0], points[0][1], 1] ));
            // Formulaicly add all other slices, closing connected faces at each step.
            let it2;
            for (let i = 1; i < pointsCount; i++) {
                it2 = 2*i;
                // add next slice
                this.positions.push(...Vec.cast( [points[i][0], points[i][1], 0], [points[i][0], points[i][1], 1] ));
                this.normals.push(...Vec.cast( [points[i][0], points[i][1], 0], [points[i][0], points[i][1], 1] ));
                // close faces
                this.indices.push(it2-2, it2-1, it2);
                this.indices.push(it2-1, it2, it2+1);
            }
            // Manually close final faces.
            let lastPos = this.positions.length - 1;
            this.indices.push(lastPos-1, lastPos, 0);
            this.indices.push(lastPos, 0, 1);
        }
    };
...
window.Shape_From_File = window.classes.Shape_From_File =
    class Shape_From_File extends Shape {
        ...
        draw(graphics_state, model_transform, material) {               // draw(): Same as always for shapes, but cancel all
